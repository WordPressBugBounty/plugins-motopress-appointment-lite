...
final class WP_Session extends Recursive_ArrayAccess implements \Iterator, \Countable {
	 ...
	 *
	 * @param $session_id
	 * @uses apply_filters Calls `mpa_wp_session_expiration` to determine how long until sessions expire.
	 */
		...
		if ( isset( $_COOKIE[ WP_SESSION_COOKIE ] ) ) {
			...
			if ( 32 < strlen( $cookie_crumbs[0] ) || ! ctype_xdigit( $cookie_crumbs[0] ) ) {
				...
				if ( time() > $this->exp_variant ) {
					$this->set_expiration();
					delete_option( "_mpa_wp_session_expires_{$this->session_id}" );
					add_option( "_mpa_wp_session_expires_{$this->session_id}", $this->expires, '', false );
				}
			}
		} else {
			$this->session_id = $this->generate_id();
	 ...
	 *
	 * @uses apply_filters Calls `mpa_wp_session_expiration_variant` to get the max update window for session data.
	 * @uses apply_filters Calls `mpa_wp_session_expiration` to get the standard expiration time for sessions.
	 */
	protected function set_expiration() {
		$this->exp_variant = time() + (int) apply_filters( 'mpa_wp_session_expiration_variant', 24 * 60 );
		$this->expires     = time() + (int) apply_filters( 'mpa_wp_session_expiration', 30 * 60 );
	}
	...
	protected function set_cookie() {
		// do nothing to make sure we do not spam php log
		// with: PHP Warning: Cannot modify header information - headers already sent
		// because of cron (fix for MPI-7906).
		if ( ! headers_sent() ) {
	...
	protected function read_data() {
		$this->container = get_option( "_mpa_wp_session_{$this->session_id}", array() );
	...
	public function write_data() {
		$option_key = "_mpa_wp_session_{$this->session_id}";

		// Only write the collection to the DB if it's changed.
		if ( $this->dirty ) {
			if ( false === get_option( $option_key ) ) {
				add_option( "_mpa_wp_session_{$this->session_id}", $this->container, '', false );
				add_option( "_mpa_wp_session_expires_{$this->session_id}", $this->expires, '', false );
			} else {
				delete_option( "_mpa_wp_session_{$this->session_id}" );
				add_option( "_mpa_wp_session_{$this->session_id}", $this->container, '', false );
			}
		}
	}
